...
export default function Contact() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!token) {
            setAlert({ message: 'You must be logged in to submit the form.', type: 'error' });
            return;
        }

        try {
            const response = await fetch('http://127.0.0.1:8000/api/create_contact/', {
                method: 'POST',
                headers: {
            ...
            if (response.status === 201) {
                ...
                setAlert({ message: 'Forbidden: You do not have permission to perform this action.', type: 'error' });
            } else {
                        ...
                        <form className="p-6 flex flex-col justify-center" onSubmit={handleSubmit}>
                            ...
                            <div className="flex flex-col mt-2">
                                ...
                                <textarea
                                    ...
                                    onChange={handleChange}
                                    className="w-full mt-2 py-8 px-3 rounded-lg bg-white border border-gray-400 text-gray-800 font-semibold focus:border-orange-500 focus:outline-none"
                                ></textarea>
                            </div>

                            <button
                                type="submit"
                                className="mt-4 py-3 px-6 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 focus:outline-none"
                            >
                                Submit
                            </button>
                        </form>
